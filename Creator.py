...
def register():
    ...
    while True:
        try:
            ...
            client = httpx.Client(headers={'authority': 'api.tellonym.me','accept': 'application/json','accept-language': 'en-GB,en;q=0.9,en-US;q=0.8','content-type': 'application/json;charset=utf-8','origin': 'https://tellonym.me','sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="100", "Microsoft Edge";v="100"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': '"Windows"','sec-fetch-dest': 'empty','sec-fetch-mode': 'cors','sec-fetch-site': 'same-site','tellonym-client': 'web:0.62.1','user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.6'}, proxies=f"http://"+GetProxies())
            res = client.post("https://api.tellonym.me/accounts/register", json={'deviceName': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.6','deviceType': 'web','lang': 'en','hCaptcha': SolveCaptcha(),'email': email,'password': password,'username': username,'limit': 25,}, timeout=30)
